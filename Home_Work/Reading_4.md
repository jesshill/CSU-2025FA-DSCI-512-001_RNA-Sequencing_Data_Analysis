# Reading #4

- Due Tuesday, December 9, 2025, 11:59 pm.
- Turn in on canvas.

## Gómez-Orte Paper

For this class, we are using data from the the paper [Effect of the diet type and temperature on the C. elegans transcriptome, Gómez-Orte et al., 2018](https://pmc.ncbi.nlm.nih.gov/articles/PMC5839384/)

**Read**

For this exercise, familiarize yourself with this study by reading
- The abstract
- The intro
- A selection of figures
- Any parts of the Materials & Methods you like

**Answer**

1. Briefly summarize Figure 1B and 1C
What are these figures trying to illustrate? How are they similar and how are they different?
You may need to do some google/wikipedia searching
You may need to consult the DESeq2 Handbook
2. Select any other figure in the paper and briefly summarize:
What is the question that is being addressed in this paper?
What is the approach that was used to address the question?
What were the findings?
What were the take-home messages?
